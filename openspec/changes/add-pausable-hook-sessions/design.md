...
For every hook `example`, the catalog publishes `example`, `example-start`, and `example-stop`. It also publishes `example-pause` and `example-resume` when the prepared metadata classifies the hook as stateful. All generated skill files use the existing exact marker body; only front matter descriptions differ. Input is accepted when Codex supplies a structured skill selection or selected-skill link whose canonical path is under Warden's generated-skill root. As a fallback for an already-open Codex task whose picker has not refreshed, a leading `$example` or `/example` token is resolved through the same registry-owned marker catalog. Unregistered commands and marker names mentioned later in prose remain inert.
